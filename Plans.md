...
**TDD RED evidence**: `bash tests/test-codex-review-session.sh` → `Unknown subcommand: review-session` / exit 1（実装前に実測）。R3修正前は同testが `initial review did not enforce an output schema`、`tests/test-orchestration-ledger.sh` が `counts review-session` でそれぞれ exit 1。
