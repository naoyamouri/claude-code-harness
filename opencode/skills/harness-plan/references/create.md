...
元の依頼、直前の会話、既存 spec、選択済み Plans.md から目的、要件、制約、決定事項と理由を回収する。既に渡された内容を聞き直さず、足りていれば Step 3 へ進む。

読み取りで補えない重要な仕様分岐だけ、具体的な案と根拠を添えて確認する。軽微な仮定は計画に明記する。計画や評価だけの依頼から、実装や外部操作の承認を推定しない。

## Step 1: 何を作るか聞く

会話と既存資料からも作るものを特定できない場合だけ質問する:
...
利用者、参考対象、完成範囲のうち、計画を変える未確定事項だけを最大3問にまとめる。参考サービスの指定など任意の入力がないことを理由に止めない。既存の依頼から作った叩き台を添え、ユーザーに調査や仕様の下書きを丸投げしない。
...
依存が不明な場合は、関連コードと task の入出力を読んで確認する。未確認を依存なしの `-` に置き換えない。解消できない依存は計画の未確定事項に残し、そのタスクの並列実行だけを保留する。重要な仕様判断が必要な場合だけユーザーに確認する。
