...
各 agent には評価対象、目的と理由、担当する視点と参照範囲、元の要求と制約、既存の証拠、完了条件を渡す。読み取り専用の評価であることと承認元を明示し、実装や外部操作を委譲しない。独立した評価だけを設定済みの同時実行上限内で並行させる。Lead は待機中も正本の照合や統合案の整理を続ける。関連する追加調査は同じ担当へ返し、最終の独立レビューは新しい文脈で行う。
...
## Step 6.5: 確定前の再調査（1 回限定）

`create.md` の Step 6.9（Plans.md 生成）でタスク案を確定する前に、採点根拠と未確定事項を確認する。
未検討の別アプローチが採用判断を変えそうな場合や、証拠の矛盾がある場合だけ、その論点を追加調査する。既に比較と必要な検証が済んでいれば、Step 6 の推奨案を維持する。

この再調査は 1 回限定。同じ plan 内で繰り返さない（ループ化禁止）。
Step 6 の採点軸・補正ルールは変更しない。
