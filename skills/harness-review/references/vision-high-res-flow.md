...
Reviewer には実際に読めた画像やページ、対象パスと範囲、元の要求、読者と目的、DoD、参照する仕様、承認元を渡す。作者の要約だけを入力の代わりにしない。新しい読み取り専用の文脈で評価し、見えないページや操作していない UI 動作は未確認と報告する。スクリーンショットで主要導線を確認できても、実操作が成功した証拠とは扱わない。
