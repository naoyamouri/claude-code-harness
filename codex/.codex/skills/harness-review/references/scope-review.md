...
- 今回の DoD に必要: 既存の承認範囲と照合し、plan への追記案または必要な判断を返す
- 今回の DoD に不要: 別 task の候補として返す

Scope Review は評価までを担当する。レビュー依頼だけで plan やコードを書き換えず、修正する担当へ元の要求、問題箇所、必要な結果、根拠を渡す。
