...
進捗を知りたいだけの依頼は読み取り専用で報告する。明示的な同期依頼では、既に許可された更新を繰り返し確認せず進める。未確定の仕様変更や承認が必要な操作は、その項目だけ保留する。
...
| チェック項目 | 検出方法 |
|------------|----------|
| 完了の候補なのに `cc:WIP` | DoD、必須検証、review artifact と実際の差分を照合 |
| 着手の候補なのに `cc:TODO` | 変更ファイルと元の依頼を照合し、他タスクの編集を除外 |
| `cc:完了` なのに未コミット | `--no-commit` の指定と完了証拠を確認 |

commit は保存された変更の証拠であり、DoD 達成やレビュー合格の証拠にはならない。未取得の検証結果を合格や未実施と断定しない。
...
差分が検出された場合は根拠を示す。同期が依頼済みなら、証拠が揃った範囲の marker を更新する。読み取り専用の進捗確認では提案までに留める:
...
| Task | 現在 | 変更後 | 理由 |
|------|------|--------|------|
| XX   | cc:WIP | cc:完了 | DoD、必須検証、独立 review の証拠を確認済み |
...
振り返り結果は理由と根拠を添えて報告する。永続記録が明示的に依頼されている場合だけ、許可された harness-mem または `.claude/agent-memory/` の保存先へ記録する。進捗確認や通常の同期から、自動的な memory 書き込みへ広げない。
