...
`gh pr create` の直後、Lead は PR 全体に対して `$harness-review code --base "$BASE_REF" --no-commit` を自動実行する。task 開始時の `BASE_REF` ではなく、必ず PR 作成後に当該 PR の `baseRefName` との merge-base を取り直す。
...
`record` は origin の current PR、review 対象 base、local HEAD、review artifact の PR base と report digest、live PR の base/head を照合する。同一 HEAD の後続 `REQUEST_CHANGES` は既存 receipt を無効化する。generic reviewer の出力や report を伴わない JSON では receipt を発行しない。`merge` は receipt head を固定し、strict protection または GitHub Free private の限定 fallback で live PR と全 CI を再確認する。認証・通信エラー、base/head 不一致、CI 未完了・失敗、review 後の更新は拒否する。
