...
各 ready batch は worker の topic branch で終える。default branch への反映は **topic branch → PR → formal review → CI → GitHub merge** のみで、loop は default branch を直接統合しない。review/CI/権限/人間判断の待機は `cc:blocked [reason]`、GitHub merge receipt 後に `harness-sync` が別 marker PR で `cc:完了 [merge-sha]` を記録する。
